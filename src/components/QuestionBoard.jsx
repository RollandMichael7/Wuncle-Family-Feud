...
import LabeledPrism from "./LabeledPrism";

export default function QuestionBoard(props) {
  const { t } = useTranslation();
  return (
    <div className="flex flex-row items-center justify-center">
      <div className="grid rounded-3xl px-10 lg:grid-flow-col lg:grid-rows-4" style={{ marginTop: "6rem" }}>
        {props.round.answers.map((x, index) => (
          <LabeledPrism
            isVisible={true}
            currentSide={PrismSide.Front}
            frontLabel={index + 1}
            bottomLabel={x.ans}
          ></LabeledPrism>
          // <div
